...
def defects_section(report: dict) -> list[str]:
    m = {x["mode"]: x for x in report["memory"]}
    return [
        "## 7. Дефекты hw2-broken",
        "",
        "**1. Хуки.** Не вызывался `remove()` — второй прогон копил хуки.",
        "",
        "**2. Tied embeddings.** `remove_duplicate=False` — lm_head считался дважды, +~26%.",
        "",
        "**3. Память.** `PeakMemory` снимал RSS после gc, режимы не отличались.",
        "",
        "**4. MPS.** `device_allocated_bytes` брал RSS вместо `torch.mps.driver_allocated_memory`.",
        "",
        f"После правок: inference {m['inference']['peak_mb']:.0f} / "
        f"LoRA {m['lora']['peak_mb']:.0f} / full FT {m['full_ft']['peak_mb']:.0f} МБ.",
        "",
    ]
