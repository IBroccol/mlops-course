    ...
    if getattr(model.config, "tie_word_embeddings", False):
        for name, param in model.named_parameters():
            if "embed_tokens" in name:
                rows.append({
                    "name": name.replace("embed_tokens", "lm_head"),
                    "shape": tuple(param.shape),
                    "numel": param.numel(),
                    "tied": True,
                })
...
def forward_hooks(modules: dict) -> tuple[dict[str, list[float]], list]:
...
def read_device_peak(device: torch.device) -> tuple[int, str]:
    if device.type == "cuda":
        return torch.cuda.max_memory_allocated(device), "torch.cuda.max_memory_allocated"
    if device.type == "mps":
        return torch.mps.driver_allocated_memory(), "torch.mps.driver_allocated_memory"
    return peak_rss()
...
class PeakMemory:
    ...
    def __exit__(self, *exc) -> bool:
        gc.collect()
        self.used, self.metric_source = read_device_peak(self.device)
        return False
    ...
    def result(self) -> dict:
        ...
        return {
            ...
            "peak_device_mb": round(self.used / 1024 ** 2, 1) if accelerator else round(rss / 1024 ** 2, 1),
            "peak_rss_mb": round(rss / 1024 ** 2, 1),
            "metric": (f"аллокатор {self.device.type}" if accelerator else "RSS процесса"),
            "metric_source": self.metric_source if accelerator else rss_source,
